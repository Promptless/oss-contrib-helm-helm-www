...
:::note
In the plain-text (table) output, Helm renders the timestamps shown by `helm status` (the `LAST DEPLOYED` line), `helm history` (the `UPDATED` column), and `helm list` (the `UPDATED` column) in your local time zone.
Helm derives the local time zone from the `TZ` environment variable, which accepts IANA time zone names such as `America/New_York` or `UTC`.
For example, `TZ="America/New_York" helm status happy-panda` displays the timestamps in Eastern time.
When `TZ` is unset, Helm uses the host's configured system time zone, which is often `UTC` in containers and continuous integration environments.
The `-o json` and `-o yaml` outputs are unaffected and preserve the stored time zone offset.
:::
...
Helm renders the `UPDATED` column in your local time zone; see the note in the `helm status` section above.
...
As noted earlier in the `helm status` section, Helm renders the `UPDATED` column in your local time zone.
