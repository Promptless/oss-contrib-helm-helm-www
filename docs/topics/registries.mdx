...
:::note
Helm writes the interactive `Username:`, `Password:`, and `Token:` prompts to stderr, so they don't pollute stdout when you redirect or capture it.
To authenticate without prompts, we recommend `--password-stdin` to read the password or identity token from stdin.
You can also pass credentials with `-u`/`--username` and `-p`/`--password`, but a secret passed with `-p`/`--password` is exposed in your shell history and the process list.
